    ...
    const display = (education) => {
        ...
        switch(education.education_type){
            case 'Tertiary':
                educationTypeContent = (
                    <div className={classes.content} >
                            <h2>{education.end_year ? `${education.start_year} - ${education.end_year}` : "Till Date"}</h2>
            ...
            case 'College': 
                educationTypeContent = (
                    <div className={classes.content} >
                            <h2>{education.end_year ? `${education.start_year} - ${education.end_year}` : 
                            `${education.start_year} - Till Date`}</h2>
                            <p>Institution: {education.institution}</p>
                            <p>Degree Obtained: {education.degree}</p>
                    </div>
                ); break;

            default: educationTypeContent = null;
        }

        return educationTypeContent
    }

    const sortedEducationHistory = [...props.educationHistory].sort((a,b) => b.end_year - a.end_year)
            ...
            <div className={classes.timeline}>
                {sortedEducationHistory.map((education, index) => 
                    <div className={`${classes.container}  ${direction}`}key={index}>
                        {display(education)}
                        <p hidden>{direction===classes.left ? direction=classes.right : direction=classes.left}</p>
                    </div> 
                )}
            </div>
        </div>
